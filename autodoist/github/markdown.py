...
def render_as_markdown(todoist_objects: ConcreteTodoistObjects) -> str:
    ...
    def _gather_attributes(todoist_objects: ConcreteTodoistObjects) -> Dict[str, List[str]]:
        attributes = {}
    ...
    for item in todoist_objects.get_all_items():
        ...
        for field_name in all_fields:
            if field_name in item.to_dict():
                row += item.to_dict()[field_name] if item.to_dict()[field_name] is not None else ""
            row += " | "
        markdown += row + "\n"
